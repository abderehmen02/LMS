...
export const QuestionForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(
        `/api/courses/${chapterId}/quiz/${quizId}/questions`,
        values
      );
  ...
  const onReorder = async (updateData: { id: string; position: number }[]) => {
    try {
      ...
      await axios.put(
        `/api/courses/${chapterId}/quiz/${quizId}/questions/reorder`,
        {
          list: updateData,
        }
      );
  ...
  const onEdit = (id: string) => {
    router.push(`/teacher/courses/${chapterId}/quiz/${quizId}/questions/${id}`);
  };
